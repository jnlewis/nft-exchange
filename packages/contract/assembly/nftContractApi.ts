import { logging, ContractPromise } from "near-sdk-as";
...
export class NFTContractApi {
   ...
   */
   transfer(tokenContract: string, tokenId: string, toAccountId: string): ContractPromise {
    ...
    let promise = ContractPromise.create<TransferArgs>(tokenContract, "nft_transfer", args, 30000000000000);
  ...
  tokensForOwner(tokenContract: string, accountId: string): ContractPromise {
    ...
    let promise = ContractPromise.create<Uint8Array>(tokenContract, "nft_tokens_for_owner", args.encode(), 30000000000000);
